...
object Patcher {
  ...
  trait JarFilePatcherTask {
    def enableMultiRelease: Boolean = true

    def patch(entry: JarEntry, bytes: Array[Byte], log: Logger): Seq[(JarEntry, Array[Byte])]

    /**
     * Creates a task that produces a patched version of a specific JAR file.
     * The task searches a project's runtime dependencies for the JAR file and
     * creates a patched version in the project's 'target/patched' directory
     * (unless the patched version already exists).
     */
    def apply(jarPrefix: String): Def.Initialize[Task[Seq[File]]] =
      Def.task {
        val log = sLog.value
        ...
        dependencyJars.map(_.data).filter(f => f.name.startsWith(jarPrefix)).map {
          originalJar =>
            ...
            if (!patchedJar.exists()) {
              ...
              patchJar(originalJar, temporaryJar, log) match {
                case Success(_) =>
    ...
    private def patchJar(inputJarFile: File, outputJarFile: File, log: Logger): Try[Unit] = {
      Using.Manager { use =>
        ...
        lazyJarEntries(jis).foreach { entry =>
          ...
          patch(entry, inputBytes, log).foreach {
   ...
   * adding a dependency on one or more instances of the [[JarFilePatcherTask]] above.
   *
   ...
   */
  object HadoopClientApiPatcherTask extends JarFilePatcherTask {
    ...
    override def patch(
      ...
      if ("org/apache/hadoop/security/UserGroupInformation.class".equals(entry.getName)) {
        ...
        ensureMatchingDigest(bytes, "md5:a6be5d7798cda1e3e854feb0edbdaacf", "input")

        val outputBytes = patchUserGroupInformationClass(bytes)
        ...
        Seq((outputEntry, outputBytes), (entry, bytes))
      } else {
        Seq((entry, bytes))
      }
    }
   ...
   */
  object SparkUnsafePatcherTask extends JarFilePatcherTask {
    ...
    override def patch(
      ...
      // Include patched Platform class for Java 26+, which will pick
      // it up from the version-specific location in multi-release jar.
      if ("org/apache/spark/unsafe/Platform.class".equals(entry.getName)) {
        log.debug(s"processing byte code in $entry")
        ensureMatchingDigest(bytes, "md5:4e794b76d4071d15f2de492b93d15a01", "input")

        val outputBytes = patchPlatformClassInitializer(bytes)
