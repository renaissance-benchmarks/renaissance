...
public final class MnemonicsCoderWithStream {
  ...
  private final Map<Character, String> mnemonics =
  ...
  /**
   * Invert the mnemonics map to give a map from chars 'A' ... 'Z' to '2' ... '9' e.g. Map(E -> 3, X
   * -> 9, N -> 6, T -> 8, Y -> 9,...)
   */
  private Map<Character, String> charCode =
      mnemonics
          .entrySet()
          .stream()
          .flatMap(
              e ->
                  e.getValue()
                      .chars()
                      .mapToObj(c -> (char) c)
                      .collect(Collectors.toMap(c -> c, c -> String.valueOf(e.getKey())))
                      .entrySet()
                      .stream())
  ...
  /**
   * A map from digit strings to the words that represent them, e.g. "5282" -> Set("Java", "Kata",
   * "Lava", ...)
   */
  ...
  /**
   * Return all ways to encode a number as a list of words Here, input a digit string that need to
   * be encoded and a set of word phrases that have been translated in previous recursion. Return a
   * set of word phrases, including the previous translated set and the word mapped from the digit
   * number in this round.
   */
  Set<String> encode(String number, Set<String> currentResult) {
    /**
     * If a digit string maps to two dict words, should return all possible words, e.g.
     * ['222':['abc','adc'] ]
     */
    return wordsForNum(words)
        .entrySet()
        .stream()
        ...
        .map(
            x -> {
              ...
              Set<String> intermediate_res =
                  encode(
                      number.substring(0, (number.length() - x.length())),
              ...
              /** Return the word phrase with a blank between two dict words e.g. "Scala rocks" */
              else
