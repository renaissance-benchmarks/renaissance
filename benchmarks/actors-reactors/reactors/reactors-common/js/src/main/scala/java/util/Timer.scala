...
class Timer(private val name: String, private val isDaemon: Boolean) {
  ...
  private def acquire(task: TimerTask): Unit = {
    ...
    if (task.owner != null || task.canceled) {
  ...
  private def scheduleFixed(
    ...
    task.timeout(delay.millis) {
      def loop(scheduledTime: Long): Unit = {
        ...
        val nowTime = System.nanoTime / 1000000
      ...
      loop(System.currentTimeMillis + period)
    }
  }
