...
public final class Platform {
  ...
  public static ByteBuffer allocateDirectBuffer(int size) {
    //
    // The limit is enforced in DirectByteBuffer(int), the primary constructor, with the
    // help of the 'java.nio.Bits' class. The constructor calls the Bits.reserveMemory()
    // method, which tracks direct memory allocations and fails if the allocated total
    // exceeds the value of MAX_MEMORY.
    //
    // The value of MAX_MEMORY comes from VM.maxDirectMemory(), which should be either the
    // value provided via -XX:MaxDirectMemorySize, or Runtime.getRuntime().maxMemory() (max
    // heap size) by default.
    //
    // https://github.com/openjdk/jdk11/blob/master/src/java.base/share/classes/jdk/internal/misc/VM.java#L121
    // https://github.com/openjdk/jdk/blob/jdk25/src/java.base/share/classes/jdk/internal/misc/VM.java#L132
    //
    // We should not be hitting those limits in Renaissance, especially when running with
    // reasonable heap sizes, so we will simply allocate memory using standard means to
    // avoid digging in the DirectByteBuffer internals.
    //
    try {
      return ByteBuffer.allocateDirect(size);
    } catch (OutOfMemoryError oome) {
      throw new OutOfMemoryError("Failed to allocate direct buffer (" + oome.getMessage() +
          "); try increasing -XX:MaxDirectMemorySize");
    }
  }
